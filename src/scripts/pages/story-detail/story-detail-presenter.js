  ...
  async fetchStoryDetail() {
    if (!this.#storyId) {
      if (typeof this.#view.displayError === 'function') {
        this.#view.displayError('Tidak dapat memuat laporan karena ID tidak ditemukan.');
      }
      return;
    }
    ...
    try {
      ...
      if (!response || typeof response.ok === 'undefined') {
        ...
        throw new Error('Respons tidak valid dari API saat mengambil detail laporan.');
      }

      if (!response.ok) {
        console.error('StoryDetailPresenter: Failed to fetch story detail (API response not OK):', response);
        if (typeof this.#view.displayError === 'function') {
          this.#view.displayError(response.message || 'Gagal memuat detail laporan.');
        }
        return; 
      }
      ...
      if (response.story) {
        ...
        if (typeof this.#view.displayError === 'function') {
          this.#view.displayError('Data detail laporan tidak ditemukan dalam respons server.');
        }
      }
      ...
      if (typeof this.#view.displayError === 'function') {
        this.#view.displayError(error.message || 'Terjadi kesalahan saat mengambil detail laporan.');
      }
    } finally {
